...
import { RecipientProfileView } from "@/components/openimpact/PersonaProfiles";
...
const NAV: DashboardNavItem[] = [
  ...
  { id: "donations", label: "Money sent to you", icon: Wallet },
  { id: "proofs", label: "What you've shown", icon: Images },
  { id: "upload", label: "Upload proof", icon: Camera },
];
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "OpenImpact" },
      {
        ...
        content:
          "Confirm funds, upload proof of use, and build a record donors can check.",
      },
    ],
  }),
...
function RecipientProfile() {
  ...
  const targetId =
    target || (needProof[0]?.id ?? answerable[0]?.id ?? "general");
  ...
  async function onConfirmAll() {
    ...
    toast.success(
      "Marked as received. The people who sent it can see that now.",
    );
  }
  ...
  async function onSubmitProof(e: React.FormEvent) {
    ...
    try {
      ...
      if (targetId === "general") {
        if (me?.orgId) {
          await submitToChain({
            action: "attachGeneralProof",
            recipientId: currentRecipientId,
          });
          await attachGeneralProof(
            currentRecipientId,
            me.orgId,
            reviewed,
            photoFile,
      ...
      if (check.flagged) {
        toast.error(
          check.reason ??
            "Uploaded, but our automated check flagged this photo for review.",
        );
      } else {
        toast.success(
          targetId === "general"
      ...
      const msg =
        "Something went wrong uploading your proof. Please try again.";
      setUploadError(msg);
      toast.error(msg);
    } finally {
      setBusy(false);
  ...
  return (
    <DashboardShell role="recipient" nav={NAV} page={page} onPageChange={go}>
      {page === "profile" && <RecipientProfileView />}

      {page === "overview" && (
        <div className="space-y-6">
          <header className="flex flex-wrap items-center gap-5 border border-border bg-card px-6 py-7 text-ink sm:px-8">
            <span
              className="grid size-16 shrink-0 place-items-center rounded-full border border-border bg-muted font-display text-2xl text-ink"
              aria-hidden
            >
              {me.name.charAt(0)}
            </span>
            <div className="min-w-0 flex-1">
              ...
              <details className="mt-3 max-w-md text-xs text-muted-foreground">
                ...
                <div className="mt-2 space-y-2 border-l-2 border-border pl-3">
                  <p>
                    <strong className="text-foreground">Pseudonym</strong>. A
                    randomly generated name like{" "}
                    <span className="data-mono">{me.pseudonym}</span> that
                    protects your real identity. Organisations, donors and the
                    public only ever see this.
                  </p>
                  <p>
                    <strong className="text-foreground">Wallet address</strong>.
                    A public account number on the blockchain (
                    <span className="data-mono">
                      {shortAddress(me.walletAddress)}
                    </span>
                    ). Funds settle here. It is not your name and cannot
                    identify you on its own.
                  </p>
                  <p>
                    Your real name is only shown here, to you. Everyone else,
                    including {org?.name ?? "your organisation"}, sees your
                    pseudonym and wallet address.
                  </p>
                </div>
              </details>
            </div>
            {established && (
              <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-muted px-3 py-1.5 text-xs font-medium text-ink">
          ...
          {!org && (
            <section className="border-2 border-pending bg-pending-soft p-6">
              <h2 className="font-display text-2xl leading-tight">
                One step left: link your organisation
              </h2>
              <p className="mt-2 max-w-xl text-sm">
                Your account works, but donations reach you through an
                organisation. Search for the one you are associated with, or
          ...
          <section className="grid gap-4 sm:grid-cols-2">
            ...
            >
              ...
              <span className="text-sm opacity-90">
                {awaiting.length > 0
                  ...
                  : "Nothing waiting right now. You are up to date."}
              </span>
            </button>
            ...
            <button
              type="button"
              onClick={() => go("upload")}
              className="flex flex-col items-start gap-3 border-2 border-border bg-card p-6 text-left transition-colors hover:bg-muted/40"
            >
              <Camera className="size-8 text-ink" aria-hidden />
              <span className="font-display text-2xl leading-tight">
                Upload proof of use
              </span>
          ...
          <section className="border border-border bg-card p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              That is how many of the donations sent to you came back with
              proof of what they bought. Donors see this number before they
              give.
            </p>
          </section>
        </div>
        ...
        <section>
          ...
          {myProofs.length > 0 ? (
            <ul className="mt-4 space-y-4">
              {myProofs.map((p) => {
                const d = p.donationId
                  ? donations.find((x) => x.id === p.donationId)
                  : undefined;
                return (
                  <li
                    key={p.id}
                    className="flex flex-wrap gap-5 border border-border bg-card p-5"
                  >
                    <img
                      src={p.photoUrl}
            ...
            <p className="mt-4 border border-dashed border-input p-8 text-center text-muted-foreground">
              Nothing uploaded yet. Once you show what a donation paid for,
      ...
      {page === "upload" && (
        <section>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            A photo, a sentence about what you bought, and a thank you if you'd
            like. That's it.
          </p>

          <form
            onSubmit={onSubmitProof}
            className="relative mt-5 space-y-6 border border-border bg-card p-6"
          >
            {busy && (
              <div className="absolute inset-0 z-10 flex items-center justify-center bg-card/80 backdrop-blur-sm">
                <div className="text-center">
                  ...
                  <p className="mt-3 text-sm font-medium">
                    Checking your photo for authenticity…
                  </p>
            ...
            <label className="block">
              ...
              >
                ...
                <option value="general">
                  Not about one donation. Post to{" "}
                  {org?.name ?? "my organisation"}'s page
                </option>
              </select>
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-4">
                ...
                {photoUrl ? (
                  ...
                  <span className="text-sm text-muted-foreground">
                    No photo chosen. We'll attach a sample stock image, which
                    the authenticity check will flag.
                  </span>
                )}
              </div>
            </div>
            ...
            <label className="block">
              ...
              />
              <span className="mt-1.5 block text-xs text-muted-foreground">
                This is signed <span className="data-mono">{me.pseudonym}</span>{" "}
                by default and appears on the public receipt. Contact details
                belong in the private share section below. Those go only to{" "}
                {donorLabel ?? "the donor"}, never to{" "}
            ...
            {targetId !== "general" && (
              <fieldset className="border border-dashed border-verified/40 bg-verified-soft/30 p-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Phone, email, WhatsApp or a social handle. Only the person who
                  funded this donation can see these. Your organisation never
                  will.
                </p>
            ...
            <p className="text-xs text-muted-foreground">
              Every upload runs through an automated authenticity check. It
              looks for reused images, signs of editing, and whether a receipt
              matches the amount claimed. If something looks off, the receipt is
              flagged for review rather than quietly approved.
            </p>
            ...
            {uploadError && (
              <div className="flex gap-3 border border-flagged/40 bg-flagged-soft p-4 text-sm">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Your photo and text are still here. Just press Upload again.
                  </p>
                </div>
              </div>
            )}
